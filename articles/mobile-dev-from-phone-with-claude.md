...
これでこのセッションが、スマホのClaudeアプリの**Code**タブから操作できるようになります。すでにターミナルで作業していて出かけることになった、というときは、そのセッションで`/remote-control`（`/rc`）と打てば、会話を引き継いだまま外に出せます。Remote Controlがつながればどれでも同じなので、やりやすいほうで大丈夫です。
...
Mac上のブラウザも、Claude in Chromeを使えばスマホから操作してもらえます。ただし**ログインが要るものは、結局PCの前にいないと進みません**。パスワードや2要素認証のコードを入れるところで止まるからです。

Developer PortalやApp Store Connect、Play Consoleを触る作業がこれにあたります。配信そのものはコマンドで回せても、証明書やプロファイルを作り直すところだけは持ち帰りになります。
