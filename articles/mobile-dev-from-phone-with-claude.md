...
個人で「マチリンガル」という英語学習アプリを作っています。KMP + Compose Multiplatformで、iOSは`iosSpeakApp/`という別のXcodeプロジェクトです。

このアプリは発音を採点するので、**実機で触ってみないと分からないことが多い**です。9月7日の1日だけで、パスキーの入口をどこに置くかを4回動かしました。どれも机上では「1つで足りる」と見えていて、実機で触って初めて「その画面に来る人が誰か」で決まると分かったやつです。

そうすると実機に入れる回数が増えるんですが、iOSのビルドにはMacが要ります。つまりMacの前にいないと何も進まない…と思っていたのですが、Claude CodeのRemote Controlを使うと、**スマホのClaudeアプリからMacを動かせる**ので、PCの前にいなくても回るようになりました。9月7日〜8日の1日で、実機へのインストールを8回、TestFlightへの配信を3回やっています。

<!-- TODO: Remote Control を使い始めたきっかけ（何がしんどくて調べたのか）を本人が加筆 -->
...
配信は**手元のMacの、その作業ツリーで**走ります。サブエージェントに渡したからといって別の場所で走っているわけではありません。8〜10分あると別の作業を始めたくなるのですが、build 34の配信中にスキルを作るためブランチを切ったら、`🔖 chore: ビルド番号を34に上げる`のコミットが`main`ではなくfeatureブランチに乗りました。
...
`State`が`available (paired)`のiPhoneのUDIDを控えて、ビルドして入れます。
...
上の`/merge-and-install`と`/merge-and-deploy`は、最初から在ったわけではなくて、1日で実機インストールを8回・TestFlight配信を3回、手で回したあとに畳んだものです。毎回やること（テストを回してからマージする・ログをファイルへ逃がす・報告を自分で確かめる）が同じだったので、そのままスキルにしました。
...
ここまで全部「手元のMacから」なので、CIに載せないのか？という話になるのですが、意識的に載せていません。理由は3つです。
...
やってみて一番効いたのは、**実機で触ってから直すまでの往復が短くなった**ことでした。気になったところをその場で直して、配信して、TestFlightで触る、までが外にいるあいだに終わります。Macは起動したままにしておくだけです。

ビルドが走るのは自分のMacなので、当然ながらMacが寝ていると何も起きません。そこだけ気をつければ、iOSアプリの開発でPCの前にいる必要はだいぶ減るなと思います。

<!-- TODO: 実際にどういう場所でやったか（電車・カフェなど）と、そのとき助かった具体例を本人が加筆 -->
