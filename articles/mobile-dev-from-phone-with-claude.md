...
家に置いてあるMacでClaude Codeを動かしたままにしておけば、スマホのClaudeアプリから指示するだけで、直す → 配信する → 実機で確かめる、までが外で終わります。

- **つなぎ方:** MacでClaude CodeをRemote Control付きで起動しておき、スマホのClaudeアプリのCodeタブから指示する。ビルドもテストも配信もMacで走る
- **実機での確かめ方:** 外にいるときはTestFlight経由（`/merge-and-deploy`）、Macと同じWi-Fiにいるときは直接インストール（`/merge-and-install`）
- **気をつけること:** Macを寝かせない・`claude`のプロセスを止めない、配信中は作業ツリーを触らない、終わったという報告は自分で確かめる
